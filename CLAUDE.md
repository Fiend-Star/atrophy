...
- A harness reporting a different check count than the exercise declares is an exercise bug, never a score: `gradeHarness` replaces the result with a `harnessError` naming both numbers, and clamps `passed` into `[0, totalChecks]` before anything is rendered or stored. In the drill loop a `harnessError` also does not consume a `submitPolicy: "single"` submission — a missing JDK or a `javac` error is not evidence about the user.
